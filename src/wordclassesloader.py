...
if __name__ == '__main__':
    ...
    should_handle_wordclasses, should_handle_morph = True, False
    options = getopt.getopt(sys.argv[1:], "", ["morph"])
    for opt,value in options[0]:
        if opt == "--morph":
            should_handle_wordclasses, should_handle_morph = False, True

    if should_handle_wordclasses:
        handle_wordclasses()
    if should_handle_morph:
        handle_morph()
